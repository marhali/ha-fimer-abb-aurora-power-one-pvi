"""Test the Fimer/ABB/Aurora Power-One config flow."""
...
from custom_components.fimer_abb_aurora_power_one_pvi.aurora_client import (
    AuroraClientError,
...
FAKE_COMPORTS = [SimpleNamespace(device="/dev/ttyUSB7")]

USB_SCAN_PATCH = "serial.tools.list_ports.comports"
...
async def test_serial_flow_success(hass: HomeAssistant) -> None:
    ...
    with (
        ...
        patch(
            "custom_components.fimer_abb_aurora_power_one_pvi.config_flow.validate_and_connect_serial",
            return_value=MOCK_IDENTIFIER,
        ),
        patch(
            "custom_components.fimer_abb_aurora_power_one_pvi.AuroraClient.from_serial",
...
async def test_tcp_flow_success(hass: HomeAssistant) -> None:
    ...
    with (
        patch(
            "custom_components.fimer_abb_aurora_power_one_pvi.config_flow.validate_and_connect_tcp",
            return_value=MOCK_IDENTIFIER,
        ),
        patch(
            "custom_components.fimer_abb_aurora_power_one_pvi.AuroraClient.from_tcp",
            return_value=mock_setup_client,
        ),
    ):
        result = await hass.config_entries.flow.async_init(
...
async def test_serial_flow_end_to_end_via_aurora_client(hass: HomeAssistant) -> None:
    ...
    with (
        patch(USB_SCAN_PATCH, return_value=FAKE_COMPORTS),
        patch(
            "custom_components.fimer_abb_aurora_power_one_pvi.aurora_client.AuroraClient.from_serial",
            return_value=mock_setup_client,
        ),
    ):
        result = await hass.config_entries.flow.async_init(
...
async def test_tcp_flow_end_to_end_via_aurora_client(hass: HomeAssistant) -> None:
    ...
    with patch(
        "custom_components.fimer_abb_aurora_power_one_pvi.aurora_client.AuroraClient.from_tcp",
        return_value=mock_setup_client,
    ):
        result = await hass.config_entries.flow.async_init(
            DOMAIN, context={"source": config_entries.SOURCE_USER}
